...
- **`cross_worker_plugin`**: A plugin that implements methods to be called by the `cross_master_plugin`.
- **`cross_master_plugin`**: A plugin that calls methods from the `cross_worker_plugin` and instructs it to call methods back.
...
### How It Works
1. **`cross_master_plugin`** calls methods from the **`cross_worker_plugin`** of another language module.
2. **`cross_worker_plugin`** calls methods back on the **`cross_master_plugin`**.
...
2. Build the **language modules** and **plugins** (`cross_worker_plugin` and `cross_master_plugin`).
...
3. Build the language modules and plugins individually. For example:
   ```
   cmake --build . --target cross_worker_plugin
   cmake --build . --target cross_master_plugin
   ```
...
3. Copy the built plugins and language modules to the `res` folder:
    - Place `cross_worker_plugin` and `cross_master_plugin` in `res/plugins/`.
...
1. **`cross_master_plugin`** calls a method from **`cross_worker_plugin`** to pass a string.
2. **`cross_worker_plugin`** processes the string and calls a method back on **`cross_master_plugin`** to return a modified string.
