...
Pass the same arguments to `s2launcher` that you would normally pass to `cs2`:
```bash
s2launcher.exe -dedicated -console -usercon +game_type 0 +game_mode 1 +map de_dust2
```

#### For Client-Side Modding:
To test or run Plugify on your game client, you **must** pass the `-insecure` flag to disable VAC protection. This prevents accidental bans while loading custom binaries.
