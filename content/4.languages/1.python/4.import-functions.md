...
Plugify provides a unified generator tool to automatically generate stub files for imported plugins.

::steps{level=4}
#### **Using the Online Generator**:
Visit [plugify-gen tool](https://gen.plugify.net/) to generate stub files through a user-friendly web interface. Simply upload your plugin manifest file (`.pplugin`) and select Python as the target language to generate the corresponding `.pyi` stub file.
