...
1. [x] **Must start with a letter** (`A-Z`, `a-z`).
2. [x] **Allowed characters**: alphanumeric (`A-Z`, `a-z`, `0-9`) plus `_`, `-` and `.`
3. [x] **Spaces are NOT allowed** in the folder name.
4. [x] The `.pplugin` configuration file must have the **same name** as the plugin folder.

::alert{type="info" icon="lucide:info"}
The extension name is derived from the manifest filename, so it must match the pattern defined in the [plugin JSON schema](https://github.com/untrustedmodders/plugify/blob/main/schemas/plugin.schema.json).
::
...
Each plugin must implement the `Plugin` interface and expose the required methods for Plugify to recognize and execute it.
...
|----------------------------|--------------------------------------------------------|---------------|
| `Plugify_PluginInit`             | Initializes the plugin and sets up required resources. | ✅ Yes        |
| `Plugify_PluginStart`            | Called when the plugin is loaded and ready to run.     | ❌ Optional   |
| `Plugify_PluginUpdate(float dt)` | Called every frame, allowing periodic updates.         | ❌ Optional   |
| `Plugify_PluginEnd`              | Called when the plugin is unloaded or shuts down.      | ❌ Optional   |

::alert{type="warning" icon="lucide:triangle-alert"}
The `Plugify_PluginInit` function is mandatory, while other methods are optional.
::

#### Using the Expose Macro

To **register your plugin with Plugify**, you must use the `PLUGIFY_PLUGIN` macro. This macro:

* Defines the plugin entry point.
* Connects your class instance (`g_examplePlugin`) to the Plugify system.
* Ensures that `Plugify_PluginInit`, `Plugify_PluginStart`, `Plugify_PluginUpdate`, and `Plugify_PluginEnd` are correctly exposed.
...
`plugin.hpp` header file provides core functionality for writing plugins. It includes:
...
* **Implement the** [Plugin](https://github.com/untrustedmodders/plugify-module-cpp/blob/main/include/plugify/plugin.hpp) **interface** to define plugin behavior.
* **Expose your plugin** using the `PLUGIFY_PLUGIN` macro.
...
::code-group
```json [plugin_name.pplugin (excerpt)]
"dependencies": [
    {
        "name": "polyhook",
        "optional": false,
...
Once your plugin loads and runs correctly, it's ready for use. If something isn't working, see the [Debugging](/languages/cpp/debugging) guide.
