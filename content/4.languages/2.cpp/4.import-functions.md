...
Visit [plugify-gen tool](https://gen.plugify.net/) to generate header files through a user-friendly web interface. Simply upload your plugin manifest file (`.pplugin`) and select C++ as the target language to generate the corresponding `.hpp` header file.
...
- The tool will generate a header file (e.g., `plugin_from_another_language.hpp`) in the specified output folder.
