...
```json [js_example_plugin.pplugin]
{
  ...
  "license": "MIT",
  "entry": "js_example_plugin.ExamplePlugin",
  "platforms": [],
  "language": "javascript",
  "dependencies": [],
  "methods": [],
...
* `entry`: Specifies the **module path** and **class name** of the plugin. For example, if your plugin is in `js_example_plugin.mjs` and the class is `ExamplePlugin`, the entry point should be `js_example_plugin.ExamplePlugin`.
* `language`: Should be set to **javascript** for JavaScript plugins.
...
Once your plugin loads and runs correctly, it's ready for use. If something isn't working, see the [Debugging](/languages/javascript/debugging) guide.
