...
Visit [plugify-gen tool](https://gen.plugify.net/) to generate type files through a user-friendly web interface. Simply upload your plugin manifest file (`.pplugin`) and select JavaScript/TypeScript as the target language to generate the corresponding `.d.ts` type definition file.
...
**Example usage**:
```bash
plugify-gen -manifest ./plugins/MyPlugin/MyPlugin.pplugin -output ./types/ -lang v8
```
