...
Visit [plugify-gen tool](https://gen.plugify.net/) to generate header files through a user-friendly web interface. Simply upload your plugin manifest file (`.pplugin`) and select Go as the target language to generate the corresponding `.go` source files.
