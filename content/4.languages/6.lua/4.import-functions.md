...
Visit [plugify-gen tool](https://gen.plugify.net/) to generate type files through a user-friendly web interface. Simply upload your plugin manifest file (`.pplugin`) and select Lua as the target language to generate the corresponding `.lua` annotation file.
