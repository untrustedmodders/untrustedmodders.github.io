...
::alert{type="warning" icon="lucide:construction"}
**The D language module is not released yet.** These pages describe the planned API and the `plugify-module-dlang` repository is not yet public. They are published for early feedback — the documented behaviour may change, and you cannot install the module today.
::
...
Once your plugin loads and runs correctly, it's ready for use. If something isn't working, see the [Debugging](/languages/dlang/debugging) guide.
