...
let kv = test_keyvalues::Kv1Create(&PlgString::from("Config"));
test_keyvalues::Kv1SetName(kv, &PlgString::from("ServerConfig"));
...
let kv = test_keyvalues::KeyValues::new(&PlgString::from("Config")).unwrap();
kv.SetName(&PlgString::from("ServerConfig")).unwrap();
...
```rust
fn process_config() {
    let kv = test_keyvalues::KeyValues::new(&PlgString::from("ServerConfig")).unwrap();
    kv.SetName(&PlgString::from("Production")).unwrap();
...
```rust
let kv1 = test_keyvalues::KeyValues::new(&PlgString::from("Config1")).unwrap();
...
fn modify_config(kv: &KeyValues) {
    kv.SetName(&PlgString::from("Modified")).unwrap();
}
...
```rust
let parent = test_keyvalues::KeyValues::new(&PlgString::from("Parent")).unwrap();
let child = parent.FindKey(&PlgString::from("ChildKey")).unwrap();
...
let child = {
    let parent = test_keyvalues::KeyValues::new(&PlgString::from("Parent")).unwrap();
    parent.FindKey(&PlgString::from("Child")).unwrap()
...
```rust
let mut parent = test_keyvalues::KeyValues::new(&PlgString::from("Parent")).unwrap();
let child = test_keyvalues::KeyValues::new(&PlgString::from("Child")).unwrap();
...
// child.SetName(&PlgString::from("NewName"));  // ERROR: value used after move
```
...
fn find_config(name: &str) -> Option<KeyValues> {
    let parent = test_keyvalues::KeyValues::new(&PlgString::from("Parent")).ok()?;
    let child = parent.FindKey(&PlgString::from(name)).ok()?;
...
let kv = Arc::new(Mutex::new(
    test_keyvalues::KeyValues::new(&PlgString::from("Config")).unwrap()
));
...
std::thread::spawn(move || {
    ...
    kv.SetName(&PlgString::from("ThreadModified")).unwrap();
});
```
...
fn on_plugin_start() {
    // Create owned object
    let mut root_config = test_keyvalues::KeyValues::new(&PlgString::from("ServerConfig")).unwrap();
    root_config.SetName(&PlgString::from("Production")).unwrap();

    // Create subkeys and transfer ownership
    let database = test_keyvalues::KeyValues::new(&PlgString::from("Database")).unwrap();
    database.SetName(&PlgString::from("PostgreSQL")).unwrap();
    root_config.AddSubKey(database).unwrap();
    // database is no longer valid - ownership transferred

    let caching = test_keyvalues::KeyValues::new(&PlgString::from("Caching")).unwrap();
    caching.SetName(&PlgString::from("Redis")).unwrap();
    root_config.AddSubKey(caching).unwrap();

    // Find returns borrowed reference
    let db_config = root_config.FindKey(&PlgString::from("Database")).unwrap();
...
4. **Check for valid handles**: Especially with borrowed objects
   ```rust
   let child = parent.FindKey(&PlgString::from("Child")).unwrap();
   if child.is_valid() {
       child.SetName(&PlgString::from("NewName")).unwrap();
   }
   ```
...
5. **Don't mix ownership models**: Stick to Rust's ownership system
   ```rust
   // Good
   let kv = KeyValues::new(&PlgString::from("Config")).unwrap();

   // Avoid unless necessary
   let mut kv = KeyValues::new(&PlgString::from("Config")).unwrap();
   let raw = kv.release();
...
```rust
let child = KeyValues::new(&PlgString::from("Child")).unwrap();
parent.AddSubKey(child).unwrap();
...
```rust
let db_config = {
    let parent = KeyValues::new(&PlgString::from("Parent")).unwrap();
    parent.FindKey(&PlgString::from("Database")).unwrap()  // ERROR: parent dropped here
};

// Correct version
let parent = KeyValues::new(&PlgString::from("Parent")).unwrap();
let db_config = parent.FindKey(&PlgString::from("Database")).unwrap();
