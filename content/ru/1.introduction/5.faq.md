...
1. **Для пользователей**:
   ...
   - Изучите автономные реализации, такие как [S2-Launcher](https://github.com/untrustedmodders/plugify-source2-launcher) или [MM2-Plugify](https://github.com/untrustedmodders/plugify-metamod-loader), чтобы увидеть Plugify в действии.
...
2. **Для разработчиков**:
   ...
   - Обратитесь к [документации](/ru/essentials/installation) для получения подробных инструкций по интеграции.
::
...
- **C#**: Поддержка среды выполнения .NET. *(Модуль Mono устарел и больше не поддерживается.)*
...
#content
Наше сообщество публикует плагины на официальном [сервере Discord](https://discord.gg/rX9TMmpang). Вы также можете изучить репозиторий [Example Plugins](https://github.com/untrustedmodders/plugify/tree/main/examples) для ознакомления с примерами плагинов и языковых модулей.
::
...
3. Поместите скомпилированный модуль в папку `res/extensions`.

Обратитесь к [Руководству по разработке языковых модулей](/ru/developer-guides/writing-language-module) для получения подробных инструкций.
::
...
#content
Plugify включает в себя **Mamba Менеджер пакетов**, который обрабатывает:
...
3. **`plugify.pconfig` (Манифест конфигурации ядра)**:
   - Определяет глобальные настройки для ядра Plugify, такие как базовый каталог, уровень логирования и тп.
...
- [Сервер Discord](https://discord.gg/rX9TMmpang): Получайте помощь в режиме реального времени от сообщества и разработчиков.
...
Обратитесь к разделу [Требования](/ru/introduction/requirements) для получения дополнительной информации.
::
...
3. Если вы установили Plugify системно, используйте менеджер пакетов Mamba или удалите установленные файлы вручную.
::
