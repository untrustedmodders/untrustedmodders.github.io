...
Передавайте лаунчеру `s2launcher` те же аргументы, которые вы обычно передаете для `cs2`:
```bash
s2launcher.exe -dedicated -console -usercon +game_type 0 +game_mode 1 +map de_dust2
```

#### Для модификации клиента
Чтобы запустить Plugify на вашем игровом клиенте, вы **обязательно** должны передать флаг `-insecure`.
::
