    ...
    def plugin_start(self):
      ...
      s2.AddConsoleCommand("custom_command", "A command is registered during OnPluginStart", flags,
                           lambda caller, context, arguments: (
                             s2.ResultType.Handled if caller == -1 else s2.PrintToServer("Custom command called.\n") or s2.ResultType.Handled
                           ), s2.HookMode.Post)
  ```
  ::
...
```csharp
public ResultType OnCommand(int caller, CommandCallingContext context, string[] arguments)
{
    Console.Write($@"
